...
def UserKerasModel(request):
    try:
        ...
        # Additional subprocess call if needed
        subprocess.call(["python", "kerasmodel.py", "--mode", "display"])

        if p.returncode != 0:
            # Log the error or handle it as needed
            error_message = f"Error: {err.decode('utf-8')}"
            print(error_message)
            return HttpResponse(error_message)  # Return the error as a response for debugging
        else:
            # Optionally, log the output
            output_message = f"Output: {out.decode('utf-8')}"
            print(output_message)

        return render(request, 'users/UserLiveHome.html', {"output": out.decode('utf-8')})
...
def UserKnnResults(request):
    ...
    df.rename(columns={'Target': 'Target', 'ECG(mV)': 'Time Chatted', 'EMG(mV)': 'Interruption', 'Foot GSR(mV)': 'Not_Engaged', 'Hand GSR(mV)': 'Physical Demand', 'HR(bpm)': 'Performance', 'RESP(mV)': 'Confused', }, inplace=True)
    data = df.to_html()
